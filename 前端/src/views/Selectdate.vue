...
export default {
  setup() {
    ...
    function getCalendar() {
      axios
        .get("/api/orders/getCalender?year=" + state.year + "&month=" + state.month + "&hpId=" + state.hpId )
        ...
        .then((response) => {
          ...
          for (let i = 0; i < state.calendarArr.length; i++) {
            //返回数组的前几个元素可能为空
            if (state.calendarArr[i].date != null) {
              //1、需要将日期day单独提取出来显示
              state.calendarArr[i].day = parseInt(
                state.calendarArr[i].date.substring(8)
              );
              //2、给每一个日期添加一个是否选中的标识（0：未选中；1：选中）
              if (state.calendarArr[i].date == state.selectDay) {
    ...
    function selectDay(index){
      ...
      state.selectDay = state.calendarArr[index].date;
